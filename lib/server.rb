  ...
  def run
    while true
      ...
      when Commands::CONNECT
        ...
        puts "CONNECT #{sender[3]}/#{sender[4]}"
      when Commands::DISCONNECT
        @clients.delete(sender)
        broadcast([Commands::DISCONNECT, sender[4]].pack('n Z*'))
        puts "DISCONNECT #{sender[3]}/#{sender[4]}"
      when Commands::MESSAGE
        message = data.unpack('n Z*')[1]
        broadcast([Commands::MESSAGE, sender[4], message].pack('n Z* Z*'))
        puts "MESSAGE #{sender[3]}/#{sender[4]} #{message}"
      when Commands::DRAW
        x, y, color = data.unpack('n n n n')[1..3]
        next if @points.include?([x,y,color])
        @points << [x, y, color]
        broadcast([Commands::DRAW, sender[4], x, y, color].pack('n Z* n n n'))
        puts "DRAW #{sender[3]}/#{sender[4]} #{x} #{y} #{color}"
      when Commands::ERASE
        ...
        erased.each do |point|
          @points.delete(point)
          broadcast([Commands::ERASE, sender[4], point[0], point[1], point[2]].pack('n Z* n n n'))
          puts "ERASE #{sender[3]}/#{sender[4]} #{point[0]} #{point[1]}"
        end
      when Commands::PINGPONG
        puts "PING #{sender[3]}/#{sender[4]} #{data.inspect}"
